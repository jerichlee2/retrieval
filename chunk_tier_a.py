    ...
    *,
    ...
    while i < len(blocks):
        ...
        should_finalize = (
            ...
            or (reached_target and current_words + block_words > target_words + max(50, target_words // 8))
        )
        ...
        if should_finalize:
            ...
            # Critical fix: if overlap would cause no forward progress, drop it.
            overlap_words = sum(count_words(x) for x in overlap)
            if overlap and overlap_words >= min_chunk_words and overlap_words + block_words > max_words:
                overlap = []

            current_blocks = list(overlap)
            current_words = sum(count_words(x) for x in current_blocks)

            # If overlap was dropped and we have no carryover, make sure next loop can consume the block.
            if not current_blocks and i < len(blocks):
                continue
